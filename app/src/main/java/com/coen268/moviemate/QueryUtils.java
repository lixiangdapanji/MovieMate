...
public final class QueryUtils {
     ...
     */
    public static List<Movie> fetch(String requestUrl, String movieid) {
        // Create URL object
        URL url = createMovieUrl(requestUrl, movieid);
     ...
     * Returns new URL object from the given string URL.
     */
    private static URL createMovieUrl(String stringUrl, String movieId) {
        URL url = null;
        try {
            url = new URL(stringUrl);
        } catch (MalformedURLException e) {
    ...
    private static Movie extractMovieFromJson(String movieJSON) {
        ...
        try {
            ...
            movie = new Movie(id, title, overview, releaseDate, posterPath, trailerPath, subscribeAvailable);
            ...
            for (int i = 0; i < videos.length(); i++) {
                ...
                if (type.equals("Trailer") && site.equals("YouTube")) {
                    ...
                    movie.trailerPath = videoId + " " + key + " " + id;
                    break;
                }
            }
            return movie;
